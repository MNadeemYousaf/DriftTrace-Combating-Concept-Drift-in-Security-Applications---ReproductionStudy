...
def contrastive_loss(z, labels, margin=MARGIN, similar_ratio=SIMILAR_RATIO):
    """
    Equation 2 with corrected similar-ratio pair sampling.
    CADE run_ids_cade.sh: --similar-ratio 0.25

    PREVIOUS IMPLEMENTATION WAS WRONG:
      With 3 balanced classes, natural positive ratio = 33%.
      To reach 25%, limiting negatives does not work (not enough negatives).
      n_neg_target = n_pos * 3 = 32,130  >  n_neg = 21,675
      → used ALL pairs → ratio stayed at 33%, identical to all-pairs.

    CORRECT IMPLEMENTATION:
      Keep ALL negatives; subsample POSITIVE pairs instead.
      n_pos_target = n_neg * (0.25/0.75) = 21,675/3 = 7,225
      Actual ratio = 7,225 / (7,225 + 21,675) = 25% ✓

    Fewer positive pairs = weaker same-class compression gradient
    → larger IQR → bounds wide enough for test-time samples.
    """
    n = z.size(0)
    if n < 2:
    ...
    # Use all negatives; subsample positives to achieve target ratio
    # similar_ratio = n_pos_used / (n_pos_used + n_neg_used)
    # n_pos_used / n_neg_used = similar_ratio / (1 - similar_ratio) = 1/3
    n_neg_used   = n_neg
    n_pos_target = max(1, int(n_neg_used * similar_ratio / (1.0 - similar_ratio)))
    n_pos_used   = min(n_pos, n_pos_target)

    perm_pos    = torch.randperm(n_pos, device=z.device)[:n_pos_used]
    sampled_pos = pos_idx[perm_pos]

    l_pos = sq_d[sampled_pos].sum()
    l_neg = torch.clamp(margin - sq_d[neg_idx], min=0.0).sum()

    total = n_pos_used + n_neg_used
    return (l_pos + l_neg) / total
