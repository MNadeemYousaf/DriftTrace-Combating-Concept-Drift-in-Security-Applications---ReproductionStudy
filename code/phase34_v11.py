"""
DriftTrace reproduction -- PHASE 3+4  (V11)
...
Writes: model_v11_<class>.pt, phase3_v11_results.json,
        phase4_v11_explanation.json, tsne_v11.png
=======================================================================
"""
...
def contrastive_loss(z, labels, margin=MARGIN):
    """
    Eq. 2 — EXACT DriftTrace implementation:
      L = mean over all pairs of [y_ij * d + (1-y_ij) * max(0, m-d)]
      d = ||z_i - z_j||_2  (regular Euclidean distance, L2 norm)

    No similar_ratio. No pair subsampling. No weighting.
    Class balance is handled entirely by the DataLoader (BALANCED=True).
    Gradient clipping in the training loop prevents NaN at initialisation.
    """
    n = z.size(0)
    if n < 2:
    ...
    # Regular Euclidean distance — epsilon inside sqrt prevents NaN gradient
    d    = (diff.pow(2).sum(dim=1) + 1e-8).sqrt()
    same = (labels[rows] == labels[cols]).float()

    l_pos = same        * d
    l_neg = (1 - same)  * torch.clamp(margin - d, min=0.0)

    # Mean over ALL upper-triangle pairs — paper's Eq. 2
...
for holdout in CLASS_NAMES:
    ...
    # ── E. Save model ─────────────────────────────────────────────────
    mp = os.path.join(OUT_DIR, f"model_v11_{holdout.replace('-','_')}.pt")
...
fig.suptitle(
    ...
    fontsize=11, y=1.02)
plt.tight_layout()
fig_path = os.path.join(OUT_DIR, "tsne_v11.png")
...
rp_det = os.path.join(OUT_DIR, "phase3_v11_results.json")
...
print(f"  phase3_v11_results.json     -> {rp_det}")
print(f"  phase4_v11_explanation.json -> {rp_exp}")
print(f"  tsne_v11.png                -> {fig_path}")
...
print()
print("  Share the Step 7 table and tsne_v11.png.")
