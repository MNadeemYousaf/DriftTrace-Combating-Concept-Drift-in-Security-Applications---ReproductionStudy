...
SIMILAR_RATIO = 0.25   # V5: wrong -- see bug in loss   # positive-pair ratio -- from CADE run_ids_cade.sh
...
def contrastive_loss(z, labels, margin=MARGIN, similar_ratio=SIMILAR_RATIO):
    """V5: similar_ratio=0.25 WRONG -- limits negatives, but n_neg_target > n_neg
    so all negatives are used anyway -> actual ratio stays at 33%, same as all-pairs."""
    n = z.size(0)
    if n < 2:
    ...
    if n_pos == 0 or n_neg == 0:
        return torch.tensor(0.0, device=z.device)
    # BUG: n_neg_target = n_pos*3 = 32130 > n_neg = 21675 -> uses ALL negatives
    n_neg_target  = max(1, int(n_pos * (1.0 - similar_ratio) / similar_ratio))
    n_neg_sampled = min(n_neg, n_neg_target)  # always = n_neg (no effect)
    perm          = torch.randperm(n_neg, device=z.device)[:n_neg_sampled]
    sampled_neg   = neg_idx[perm]
    l_pos = sq_d[pos_idx].sum()
    l_neg = torch.clamp(margin - sq_d[sampled_neg], min=0.0).sum()
    return (l_pos + l_neg) / (n_pos + n_neg_sampled)
